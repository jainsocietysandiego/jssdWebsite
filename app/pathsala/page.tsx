...
interface LevelData {
  Level: string; // Using the exact casing from the API
  Title: string;
  Description: string;
  'Age Group': string;
  Duration: string;
...
const Pathsala: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(API_URL)
      .then((res) => {
        console.log("Raw API Response:", res.data); // <-- Log the raw response here
        if (res.data && res.data.levels) {
          ...
          console.error("API Response structure is not as expected:", res.data);
          ...
          <section className="py-20 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
                {levels.length === 0 ? (
                  <p>No levels available to display.</p>
                ) : (
                  levels.map((level) => (
                    <div key={level.Level} className="bg-white rounded-lg shadow-lg border border-orange-100 hover:shadow-xl transition-shadow">
                      <div className="p-6">
                        ...
                        <button
                          onClick={() => {
                            sessionStorage.setItem(`pathsala-level-${level.Level}`, JSON.stringify(level));
