...
const API_URL =
  'https://script.google.com/macros/s/AKfycbxGGp07f15g5nmWvEhKuYwXufkp0bfTKO40mDeiAbXoyFo83JyFNer3vK0_KKeNY7sUgA/exec';
const CACHE_KEY = 'pathshala-api';
const CACHE_TTL = 10 * 60 * 1000; // 10 min

// -------------------- SKELETON COMPONENT ------------------------
const PathsalaSkeleton = () => (
    ...
    <div className="pt-16">
      ...
      <div className="bg-gradient-to-r from-orange-600 to-orange-700 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="h-12 w-3/4 mx-auto bg-orange-200 rounded mb-6" />
          <div className="h-6 w-1/2 mx-auto bg-orange-100 rounded" />
        </div>
      </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <div className="h-8 w-1/3 mx-auto rounded bg-orange-100 mb-4" />
            <div className="h-5 w-2/3 mx-auto rounded bg-orange-50 mb-4" />
          </div>
          {/* Levels Skeleton */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-white rounded-lg shadow-lg border border-orange-100">
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="h-8 w-24 bg-orange-200 rounded" />
                    <div className="h-6 w-20 bg-orange-50 rounded" />
                  </div>
                  <div className="h-6 bg-orange-100 rounded mb-3 w-2/3" />
                  <div className="h-4 bg-orange-50 rounded mb-4 w-full" />
                  <div className="space-y-2 mb-6">
                    <div className="h-3 w-24 bg-orange-100 rounded" />
                    <div className="h-3 w-32 bg-orange-50 rounded" />
                  </div>
                  <div className="h-10 bg-orange-200 w-full rounded" />
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[...Array(3)].map((_, idx) => (
                <div key={idx} className="text-center">
                  <div className="bg-orange-600 text-white rounded-full w-16 h-16 mx-auto mb-4" />
                  <div className="h-5 w-28 mx-auto bg-orange-100 rounded mb-2" />
                  <div className="h-4 w-4/5 mx-auto bg-orange-50 rounded" />
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="h-8 w-1/3 mx-auto rounded bg-orange-100 mb-4" />
            <div className="h-5 w-2/3 mx-auto rounded bg-orange-50" />
          </div>
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <div className="h-6 w-3/5 bg-orange-100 mb-6 rounded" />
              ...
              <div>
                <div className="h-6 w-1/2 bg-orange-100 mb-6 rounded" />
                <div className="space-y-4">
                  {[...Array(3)].map((_, idx) => (
                    <div key={idx} className="flex items-start">
                      <div className="bg-orange-600 rounded-full w-8 h-8 mr-3 mt-1" />
                      <div className="h-5 w-2/3 bg-orange-50 rounded" />
                    </div>
                  ))}
                </div>
                <div className="mt-6 h-10 w-full bg-orange-200 rounded" />
              </div>
            </div>
          </div>
        </div>
...
const Pathsala: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Always fetch in background. Cache updated if fresh.
    axios
      .get(API_URL)
      .then((res) => {
        if (res.data.levels) {
          ...
          localStorage.setItem(
            CACHE_KEY,
            JSON.stringify({ data: res.data.levels, timestamp: Date.now() })
          );
        }
        setLoading(false);
      })
    ...
    // On first visit, show loader until fetch completes. On reload/cache, loading is false unless cache is corrupt.
  }, []);

  if (loading) return <PathsalaSkeleton />;
